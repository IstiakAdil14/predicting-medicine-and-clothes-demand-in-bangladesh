# Bangladesh Clothes Demand Data Preprocessing Pipeline
# This script cleans and prepares raw clothes demand data for machine learning
...
# Define clothing items and numeric columns for processing
...
def main():
    ...
    # Remove negative values in clothes demand (data quality issue)
    for col in clothes_cols:
        if col in df.columns:
            df.loc[df[col] < 0, col] = np.nan

    # Fill missing clothes demand using hierarchical median imputation
    for col in clothes_cols:
        if col in df.columns:
